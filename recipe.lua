...
function technic_sawmill.register_sawmill_recipe(data)
...
-- 	table.insert(recipes, {"ethereal:scorched_tree", "???"})
	...
	-- undo-specific additions
	if ethereal.mod and ethereal.mod == "undo" then
		table.insert(recipes, {"ethereal:olive_trunk", "ethereal:olive_wood 8"})
	end
...
for _, data in pairs(recipes) do
	if data[3] then
		technic_sawmill.register_sawmill_recipe({input = {data[1]}, output = data[2], time = data[3]})
	else
		technic_sawmill.register_sawmill_recipe({input = {data[1]}, output = data[2]})
	end
end
